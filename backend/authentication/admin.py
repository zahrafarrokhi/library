...
class UserCreationForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if pss and pss != "":
            self.cleaned_data['unhashed_password'] = pss
        else:
            self.cleaned_data['unhashed_password'] = None

    def save(self, commit=True):
        # Save the provided password in hashed format
        user = super(UserCreationForm, self).save(commit=False)
        new_password = self.cleaned_data.get("unhashed_password", None)
        if new_password:
            user.set_password(self.cleaned_data["unhashed_password"])
        if commit:
            user.save()
        return user
